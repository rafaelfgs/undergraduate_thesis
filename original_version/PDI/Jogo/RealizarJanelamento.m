...
if ~isempty(Robos.Pos1)
    
    % Determina o índice do Robô mais próximo do anterior
    Janela.DistRobo1 = rssq( repmat(Janela.Pos1,size(Robos.Pos1,1),1) - Robos.Pos1 ,2);
    [~,Janela.IndRobo1] = min(Janela.DistRobo1);
    
    % Atualiza o centróide e a orientação do Robô 1
    Janela.Pos1 = Robos.Pos1(Janela.IndRobo1,:);
    Janela.OrientRobo1 = Robos.Orient1(Janela.IndRobo1);
    
end

% Verifica se foram encontrados Robôs cor2 nesse frame
if ~isempty(Robos.Pos2)
    
    % Determina índice do Robô mais próximo do anterior e atualiza o centróide
    Janela.DistRobo2 = rssq( repmat(Janela.Pos2,size(Robos.Pos2,1),1) - Robos.Pos2 ,2);
    [~,Janela.IndRobo2] = min(Janela.DistRobo2);
    
    % Atualiza o centróide e a orientação do Robô 2
    Janela.Pos2 = Robos.Pos2(Janela.IndRobo2,:);
    Janela.OrientRobo2 = Robos.Orient2(Janela.IndRobo2);
    
end

% Verifica se foram encontrados Robôs cor3 nesse frame
if ~isempty(Robos.Pos3)
    
    % Determina o índice do Robô mais próximo do anterior e atualiza o centróide
    Janela.DistRobo3 = rssq( repmat(Janela.Pos3,size(Robos.Pos3,1),1) - Robos.Pos3 ,2);
    [~,Janela.IndRobo3] = min(Janela.DistRobo3);
    
    % Atualiza o centróide e a orientação do Robô 3
    Janela.Pos3 = Robos.Pos3(Janela.IndRobo3,:);
    Janela.OrientRobo3 = Robos.Orient3(Janela.IndRobo3);
    
end

% Verifica se foram encontrados Robôs adversários nesse frame
if ~isempty(Robos.Adv)
    
    % Determina as distâncias entre os adversários encontrados e os anteriores
    Janela.DistAdv = [ rssq( repmat(Janela.Adv1,size(Robos.Adv,1),1) - Robos.Adv ,2)';
                       rssq( repmat(Janela.Adv2,size(Robos.Adv,1),1) - Robos.Adv ,2)';
                       rssq( repmat(Janela.Adv3,size(Robos.Adv,1),1) - Robos.Adv ,2)' ];
    ...
    try Janela.Adv1 = Robos.Adv(Janela.IndAdv(1),:); catch, end
    try Janela.Adv2 = Robos.Adv(Janela.IndAdv(2),:); catch, end
    try Janela.Adv3 = Robos.Adv(Janela.IndAdv(3),:); catch, end
    
end

% Verifica se foram encontradas Bolas nesse frame
if ~isempty(Robos.Bola)
    
    % Determina o índice da Bola mais próxima da anterior e atualiza a posição
    Janela.DistBola = rssq( repmat(Janela.Bola,size(Robos.Bola,1),1) - Robos.Bola ,2);
    [~,Janela.IndBola] = min(Janela.DistBola);
    
    % Atualiza o centróide da Bola
    Janela.Bola = Robos.Bola(Janela.IndBola,:);
